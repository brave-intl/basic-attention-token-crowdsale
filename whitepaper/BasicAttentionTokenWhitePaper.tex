...
\label{sec-6-1}
{Our goal is to raise a maximum of \$24 million USD and a minimum of \$6 million USD. Some of the numbers may change with ETH/USD exchange rates and volatility, but the following numbers are best effort estimates as of May 23, 2017. }%%\today .}

\begin{itemize}
\item{\textbf{Maximum financing}: 181,818.2 ETH -this may change with exchange rates.}
\item{\textbf{Minimum financing}:  45,454.6 ETH -this may also change with exchange rates.}
\item{\textbf{Exchange rate}: 1 ETH = 5,500 Basic Attention Tokens (\textrm{BAT}) -this may change with ETH exchange rates.}
\item{\textbf{Token contract address}: TBD (Published through various channels 48hrs before crowdsale launch date).}
\item{\textbf{Launch date and time}: 8AM PST May 31, 2017 (based on Blocknumbers)}
\item{\textbf{Token launch time-frame}: 30 days (based on Blocknumbers).}
\item{\textbf{Token launch completion}: Token launch will end when either the maximum number of ETH are raised or 30 days have expired. If less than the minimum ETH are raised, ETH can be retrieved by holders of BAT.}
\end{itemize}
...
BAT will be a fixed ratio to ETH. This may vary slightly with ETH volatility as we get closer to the contract deployment date. At present ETH to USD exchange rates, 5500 \textrm{BAT} per ETH gives the correct crowdsale parameters for 1 billion \textrm{BAT} offered for sale.\\
\\
}
